...
const host='https://koreanbj.club'
const fs = require('fs');
...
const resHref=function (str) {
    ...
    return  str.replace(/http+.(com|club)/gi,'')
}
...
const thudam={
   ...
   async getRate(uri){
    ...
    async detail(uri){
        let list={docs:[],meta:{},video:{thumb:[],docs:[],tags:[]},nav:[]},_this=this,curi=uri.match(/[a-z]+\.[a-z]{2,6}/gi) ? uri :host+encodeURI(uri);
         console.log(curi,'detail')
        return  new Promise((resolve, reject) => {
            pup.queue([{
                uri:curi,
                encoding:null,
                callback: function (error, res, done) {
                    if(error){
                      ...
                      let str= $('html').html().match(/https:\/\/cdn\.koreanbj\.club\/[^\.]+\.mp4/gi)
                        if(str){
                            list.video.url=str[0]
                        }
                        $('[data-video-uid]').each(function (k,v) {
                            if($(this).find('a').attr('href')){
                                let json={
                                    href:baseUrl+resHref($(this).find('a').attr('href')),
                                    title:purHtml($(this).find('a').attr('title')),
                                    img:$(this).find('img').attr('data-src') || $(this).find('img').attr('src'),
                                }
...
/*thudam.detail('/bj-qhfk64-2/').then(res =>{
console.log(res)
})*/
