...
const purHtml=function(str,s){
    ...
    str=str.replace(/\|.+/gi,'')
    str=str.replace(/18porn\.cc|好色™/gi,'pornavd')
    str=str.replace(/[\t\v\n\r\f]/g, "")
...
const thudam={
    ...
    async list (uri=host){
        let list={docs:[],meta:{},range:[]},_this=this,
        curl=uri.match(/http.+\.[a-z]{2,6}/gi) ?  uri : host+uri;
        console.log(curl,'list')
        return  new Promise((resolve, reject) => {
            pup.queue([{
                uri:curl,
                method: 'GET',
                callback: function (error, res, done) {
                    if(error){
                        ...
                        $('.thumbnail').each(function () {
                            ...
                            let img=$(this).find('img').attr('data-original') || $(this).find('img').attr('src')
                            if( href && href.indexOf('video') > -1 && img){
                                list.docs.push({
                                    href:baseUrl+'/'+resHref(href),
                        ...
                        $('.pagination1').eq(0).find('li').each(function () {
                            list.range.push({
                ...
                callback: function (error, res, done) {
                    if(error){
                        ...
                        Object.assign(list.video,{
                            title:purHtml($('title').text()),
                            desc:purHtml($('[name="description"]').attr('content')),
                            img:$('#my_video').attr('poster'),
                        })
                        let src=$('source').attr('src')
                        console.log(src,'m3u8')
                        if(src){
                           let hlsUrl=src.split('/hls/')
                            list.video.url='/hls/'+hlsUrl[1]+'?siteUrl='+hlsUrl[0]
...
/*thudam.detail('/video-487761.htm').then(res =>{
  console.log(res)
})*/
